...
class JobSearchAgent:
    ...
    def __init__(self, config_file: Optional[str] = None):
        ...
        self.preferences_dir = os.path.expanduser(
        ...
        # If multiple sources were used (strategy = "all")
            ...
            for source_name in raw_results["sources"]:
                ...
                try:
                    ...
                    normalized_jobs = [
                        job_source.normalize_job(job) for job in parsed_jobs
                    ]
                    all_jobs.extend(normalized_jobs)
                    used_sources.append(source_name)
                except Exception as e:
                    logging.error(
                        f"Error processing results from {source_name}: {str(e)}"
                    )
    ...
    def enhanced_job_search(
        ...
        if "technicalSkills" in preferences and preferences["technicalSkills"]:
            ...
            relevant_skills = [
                skill
                for skill in preferences["technicalSkills"]
                if skill.lower() not in keywords.lower()
            ][:3]
            if relevant_skills:
        ...
        if "careerGoals" in preferences and preferences["careerGoals"]:
            ...
            if goal_terms:
                ...
                new_terms = [
                    ...
                    if term.lower() not in enhanced_keywords.lower()
                ][:2]
                if new_terms:
        ...
        if "jobTypes" in preferences and preferences["jobTypes"]:
            ...
            if job_types and not experience_level:
                ...
                if any(
                    jt.lower() in ["full-time", "part-time", "contract", "freelance"]
                    for jt in job_types
                ):
                    preferred_job_type = next(
                        (
                            jt
                            for jt in job_types
                            if jt.lower()
    ...
    def analyze_resume_match(
        ...
        if (
            ...
            and "choices" in raw_result
            and raw_result["choices"]
        ):
            ...
            score_match = re.search(
                r"(?:match score|score|rating|match)(?:[:\s]+)(\d{1,3})",
                analysis_text,
                re.IGNORECASE,
            )
            if score_match:
    ...
    def list_sources(self) -> List[str]:
        """
        Get a list of all registered job source names.

        Returns:
            List of source names
        """
        source_info = self.registry.get_all_source_info()
        return [info["name"] for info in source_info]
    ...
    def enable_source(self, source_name: str) -> Dict[str, Any]:
        ...
        return {"status": "error", "message": f"Job source '{source_name}' not found"}
